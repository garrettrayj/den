...
struct ImportExportUtility {
    static func importOPML(url: URL, modelContext: ModelContext, pageUserOrderMax: Int16) {
        let opmlFolders = OPMLReader(xmlURL: url).outlineFolders

        opmlFolders.enumerated().forEach { idx, opmlFolder in
            let page = Page.create(in: modelContext, userOrder: pageUserOrderMax + Int16(idx))
            page.name = opmlFolder.name
            page.symbol = opmlFolder.icon

            opmlFolder.feeds.forEach { opmlFeed in
                let feed = Feed.create(in: modelContext, page: page, url: opmlFeed.url)
